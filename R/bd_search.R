...
#' @param level administrative level of bangladeshPLUS. Should be one of: "country", "division", "district", "upazila","union'
    ...
    function(searchFor, level = "division", as.is = FALSE, coordinates = FALSE){
        ...
        switch (level,
                ...
                division = {
                    ...
                    i <- with(area_names, grepl(searchFor, paste(Division), ignore.case = T))
                    r <- area_names[i,]
                ...
                district = {
                    i <- with(area_names, grepl(searchFor, paste(District), ignore.case = T))
                    r <- area_names[i,]
                    if(nrow(r) == 0){warning("No Result Found")}
                    if(!coordinates){
                        return(r)
                ...
                upazila = {
                    i <- with(area_names, grepl(searchFor, paste(Upazila), ignore.case = T))
                    r <- area_names[i,]
                    if(nrow(r) == 0){warning("No Result Found")}
                    if(!coordinates){
                        return(r)
                ...
                union = {
                    i <- with(area_names, grepl(searchFor, paste(Union), ignore.case = T))
                    r <- area_names[i,]
                    if(nrow(r) == 0){warning("No Result Found")}
                ...
                all = {
                    i <- with(area_names, grepl(searchFor, paste(Division,District,Upazila, Union),
                                                            ignore.case = T))
                    r <- area_names[i,]
                    if(nrow(r) == 0){warning("No Result Found")}
                    if(!coordinates){
